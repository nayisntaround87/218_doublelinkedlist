...
void addNode() {
    ...
    if (START == NULL || newNode->noMhs <= START->noMhs) {
        ...
        if (START !=NULL && newNode->noMhs == START->noMhs) {
            cout << "\033[31mDuplicate roll number not numbers not allowed\033[0m" << endl;
            return;
        }
        ...
        {                            // step 1.c: traverse the list to find the correct position
            ...
            current = current->next; // step 1.e: move the current to the next mode
        }
...
void deleteNode()
{
    ...
    if (current == NULL)
    {
        cout << "\033[31mThe record with roll number " << "not found\033[0m" << endl;
        return;
    }
